...
class _CashbookViewScreenState extends State<CashbookViewScreen> {
  ...
  List<CashEntry> _filteredEntries = [];
  bool _loading = true;
  final TextEditingController _searchController = TextEditingController();

  double get _totalIn => _filteredEntries
      .where((e) => e.type == CashEntryType.inFlow)
      .fold(0.0, (s, e) => s + e.amount);
  double get _totalOut => _filteredEntries
  ...
  void _applySearch() {
    ...
    if (query.isEmpty) {
      ...
      _filteredEntries = _entries.where((entry) {
        // Search by note
        if (entry.note != null && entry.note!.toLowerCase().contains(query)) {
          return true;
        }
        // Search by amount (convert to string for search)
        final amountStr = entry.amount.toString();
